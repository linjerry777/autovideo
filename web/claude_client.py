...
Script 語氣：親切的 AI 圈友口吻（**貼近觀眾、不是新聞主播**）。
若該則內容本身有 takeaway 可以帶 why-you-care，**沒有的話不要硬套**。
- 硬套差例：「先看這招挑 AI 精準醫療」（醫療得獎不是「招」）
- 好例：「帶你看這個，AI 精準醫療拿獎了，背後是基因比對技術」（帶知識點）

CTA 語氣：驅動**關注**（權重 8×），不是評論。
- 好：「想追最新 AI 圈動態？追蹤 Doro 每天一則」
- 好：「每天一則 AI 新聞懶人包，追蹤不錯過」

**多則合輯時（>=2 則）每則 script 的結構**（避免口吻疲勞）：
第 1 則 script_short 開頭放**整體導入句**（新聞導讀式，不是教學式）：
- 範例：「帶你快速了解今天 3 件 AI 新聞。第一件…」
- 範例：「今天 AI 圈這 3 件事你不能錯過，先看這個…」
...
- **禁止用「欸」「你聽好」「跟你說」開場** — 使用者實測聽起來疲勞/網紅 AI 口音
- 語氣詞限用：「真的」「等等」「這…」等，且整支影片**至多 1 處**（不要每則都塞）
- 寧可用停頓（…）與問句，不要塞語氣詞
- 強調詞用「！」或用重複「這…這太誇張了」
...
- 好（過程式）：「Anthropic 想一統 AI 代理，但 Perplexity 直接嗆：Token 稅太貴了！」
